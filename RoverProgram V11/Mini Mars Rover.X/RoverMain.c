...
int main(void) {
  ...
  configShortcut();
  //while(1);
  for(long i = 0; i < 10000; i++); //Delay to ensure proper setup
  //while(1);
  ...
  while(1) {
    ...
    if(FAR_LEFT_QRD_STATE && MIDDLE_LEFT_QRD_STATE && MIDDLE_RIGHT_QRD_STATE && FAR_RIGHT_QRD_STATE &&   //Check if off line
            (RIGHT_DIST_STATE > SIDE_DIST_CANYON_THRESHOLD) && (LEFT_DIST_STATE > SIDE_DIST_CANYON_THRESHOLD) &&            //Check if walls detected
